...
export function ObjectionBreaker() {
  ...
  return (
    <section className="relative py-28 sm:py-36" style={{ background: "#0A0F1C" }}>
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mt-10 max-w-2xl mx-auto">
          {guarantees.map((g) => (
            <div
              key={g.title}
              className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.03)] p-5 text-left transition-all duration-200 hover:border-[rgba(59,130,246,0.15)] hover:bg-[rgba(59,130,246,0.04)]"
            >
              <g.icon className="h-5 w-5 text-[#3B82F6] mb-3" />
              <p className="text-sm font-semibold text-[#E2E8F0]">{g.title}</p>
              <p className="text-[13px] text-[#64748B] mt-1 leading-relaxed">{g.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10">
          <Button
            size="lg"
            className="gap-2.5 px-10 text-sm font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white shadow-xl shadow-[rgba(37,99,235,0.25)] h-14 rounded-xl transition-all duration-200 hover:shadow-2xl hover:scale-[1.02] border-0"
