...
import { Search, Shield, ShieldCheck, ShieldX, RotateCcw, Lock } from "lucide-react";
...
export function PermissionsEditor({ userId, isCeoOrMaster, onClose }: PermissionsEditorProps) {
  ...
  const handleToggle = (key: string) => {
    if (isPermissionLocked(key, userRole)) return;
    setLocalPerms(prev => ({
      ...prev,
  ...
  const handleResetToDefaults = () => {
    ...
    ALL_PERMISSION_KEYS.forEach(key => {
      ...
      perms[key] = {
        granted: !!defaultEntry && !isPermissionLocked(key, userRole),
                          ...
                          <AccordionContent className="pb-3">
                            <div className="space-y-3">
                              {module.permissions.map(perm => {
                                const locked = isPermissionLocked(perm.key, userRole);
                                const restrictedRoles = FIXED_PERMISSION_RULES[perm.key];
                                return (
                                  <div key={perm.key} className={`flex items-center justify-between py-1 gap-3 ${locked ? "opacity-50" : ""}`}>
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                      {locked ? (
                                        <Tooltip>
                                          ...
                                          <TooltipContent>
                                            Restrito a: {restrictedRoles?.join(", ")}
                                          </TooltipContent>
                                        </Tooltip>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      {perm.scopable && localPerms[perm.key]?.granted && !locked && (
                                        <Select
