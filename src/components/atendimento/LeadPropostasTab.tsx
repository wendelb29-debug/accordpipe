...
export function LeadPropostasTab({ lead, addActivity, signatureMode = false, onUpdateLead }: { lead: CrmLead; addActivity: (data: any) => Promise<any>; signatureMode?: boolean; onUpdateLead?: (id: string, updates: Partial<CrmLead>) => Promise<boolean> }) {
  ...
  const generateProposalPdf = async (proposal: any) => {
    ...
    const templateData: import("./ProposalTemplatePremium").ProposalTemplateData = {
      ...
      primaryColor: (srv as any)?.doc_primary_color || srv?.brand_primary_color || "#1E2952",
      secondaryColor: (srv as any)?.doc_secondary_color || srv?.brand_secondary_color || "#4F46E5",
      accentColor: (srv as any)?.doc_accent_color || srv?.brand_accent_color || "#10B981",
      bgColor: (srv as any)?.doc_bg_color || srv?.brand_bg_color || "#F8F9FC",
      textColor: (srv as any)?.doc_text_color || srv?.brand_text_color || "#1F2937",
