...
export function LeadDocsTab({ lead }: LeadDocsTabProps) {
  ...
  const handleGenerateFromTemplate = async (template: { id: string; name: string; pdf_url: string; pdf_path: string; contract_content: string | null }) => {
    ...
    try {
      ...
      // Try to load approved proposal from new Zuper proposals table (best-effort, don't fail if it errors)
      let approvedZuper: any = null;
      try {
        const { data: zuperList } = await supabase
          .from("proposals")
          .select("*")
          .eq("lead_id", lead.id)
          .order("created_at", { ascending: false });
        const list = (zuperList as any[]) || [];
        approvedZuper = list.find((p) => p.status === "aprovada") || list.find((p) => p.status === "aberta") || list[0] || null;
      } catch (e) {
        console.warn("[contract] could not load zuper proposals", e);
      }
      ...
      if (approvedZuper) {
        try {
          ...
          console.warn("[contract] could not load proposal line items", e);
        }
      }
      if (!proposal) {
