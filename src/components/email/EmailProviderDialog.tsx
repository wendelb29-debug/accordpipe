...
import { Mail, Loader2, Lock, Info, ExternalLink } from "lucide-react";
    ...
    try {
      const { error } = await supabase.from("email_accounts" as any).insert({
        ...
        status_message: isOAuth
          ? "Autenticação OAuth — conexão real será habilitada em breve"
      ...
      toast.success("Conta de e-mail cadastrada!", {
        description: isOAuth
          ? "A conexão OAuth será concluída em uma próxima atualização."
          : "A sincronização IMAP será habilitada em breve.",
      });
      reset();
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) reset(); onOpenChange(o); }}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {isOAuth && (
          <div className="mt-3 rounded-lg border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-500/10 p-3 flex gap-2.5">
            <Info className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
            <div className="text-[11.5px] text-blue-700 dark:text-blue-300 leading-relaxed">
              A autenticação <b>{providerName}</b> via OAuth (Google/Microsoft/Apple) requer configuração de credenciais do provedor e Edge Functions. Por enquanto, cadastre a conta para já preparar o ambiente — a conexão real será habilitada na próxima fase.
            </div>
          </div>
        )}
        ...
        {isImap && (
          <div className="mt-3 rounded-lg border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-500/10 p-3 flex gap-2.5">
            ...
            <div className="text-[11.5px] text-amber-700 dark:text-amber-300 leading-relaxed">
              Por segurança, use <b>senha de aplicativo</b> em vez da senha principal da conta.
              {" "}
        ...
        <div className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    className="w-full h-9 px-3 rounded-lg border border-border bg-background text-[13px] outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-900/30"
                  />
                </div>
                <div>
          ...
          <div className="border-t border-border pt-3 space-y-2.5">
            <Toggle
              label="Usar mesmo nome de remetente"
              sub="Substitui o remetente real por um nome fixo"
              checked={sharedSender}
            ...
            <Toggle
              label="Definir limite diário de e-mails"
              sub="Bloqueia envios após o limite"
              checked={!!dailyLimit}
          ...
          <div className="border-t border-border pt-3 space-y-2.5">
            <Toggle
              label="Integração com o CRM"
              sub="Vincula e-mails a leads automaticamente"
              checked={crmIntegration}
              onChange={setCrmIntegration}
            />
            <Toggle
              label="Integração com Calendário"
              sub="Sincroniza convites e eventos"
