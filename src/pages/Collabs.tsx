        ...
        <div className="flex-1 overflow-y-auto py-2 px-1.5 scrollbar-thin">
          {searchFocused && search.trim() === "" && (
            <div className="px-2 pt-1 pb-2 mb-1 border-b border-gray-100">
              <div className="text-[10.5px] font-semibold tracking-wider text-gray-400 uppercase px-1 mb-2">
                Bate-papos recentes
              </div>
              <button
                type="button"
                onMouseDown={(e) => { e.preventDefault(); openAnotacoes(); }}
                disabled={openingNotes}
                className="w-full flex flex-col items-center gap-1 px-2 py-2 rounded-xl hover:bg-sky-50 transition disabled:opacity-60"
                title="Anotações — visível apenas para você"
              >
                <div className="h-11 w-11 rounded-full flex items-center justify-center bg-sky-500 text-white shadow-sm">
                  {openingNotes ? <Loader2 className="h-5 w-5 animate-spin" /> : <Bookmark className="h-[18px] w-[18px]" fill="currentColor" />}
                </div>
                <div className="text-[11.5px] font-medium text-gray-700 leading-tight">Anotações</div>
              </button>
            </div>
          )}
          {search.trim() !== "" && (() => {
            const q = search.trim().toLowerCase();
            const matches = tenantUsers
              .filter((u) => u.id !== user?.id && (u.name.toLowerCase().includes(q) || (u.handle || "").toLowerCase().includes(q)))
              .slice(0, 8);
            if (matches.length === 0) return null;
            return (
              <div className="px-2 pt-1 pb-2 mb-1 border-b border-gray-100">
                <div className="text-[10.5px] font-semibold tracking-wider text-gray-400 uppercase px-1 mb-1.5">
                ...
                {matches.map((u) => (
                  <button
                    key={u.id}
                    type="button"
                    onMouseDown={(e) => { e.preventDefault(); setSearch(""); openDirectWith(u.id); }}
                    className="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-emerald-50/70 transition text-left"
                  >
                    {u.avatar_url ? (
                      <img src={u.avatar_url} alt="" className="h-8 w-8 rounded-full object-cover" />
                    ) : (
                      <div className="h-8 w-8 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white text-[11px] font-semibold flex items-center justify-center">
                        {initialsOf(u.name)}
                      </div>
                    )}
                              ...
                              {hasBubble && (
                                <div
                                  className={cn(
                                    ...
                                    isSent
                                      ? "text-white rounded-[22px] rounded-br-md shadow-[0_8px_24px_-10px_rgba(16,185,129,0.55)]"
                                      : "bg-white/95 text-[#1a1a2e] rounded-[22px] rounded-bl-md backdrop-blur-sm shadow-[0_4px_16px_-6px_rgba(15,23,42,0.10),inset_0_0_0_1px_rgba(16,185,129,0.06)]"
                                  )}
                                  style={isSent ? { background: "linear-gradient(135deg, #10b981 0%, #059669 100%)" } : undefined}
                                >
                                  {quote && (
                ...
                <div className="px-6 pb-4">
                  ...
                  <div className="rounded-xl border border-gray-200 p-3 bg-white">
                    ...
                    <div className="relative mt-3">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
                      <input value={memberSearch.trim()} onChange={(e) => setMemberSearch(e.target.value)} placeholder="Buscar usuários do tenant..." className="w-full rounded-lg bg-gray-50 border border-transparent pl-9 pr-3 py-2 text-sm outline-none focus:border-blue-300 focus:bg-white" />
                    </div>
                    {memberSearch && (
                      <div className="mt-2 max-h-[220px] overflow-y-auto rounded-lg border border-gray-100 divide-y divide-gray-50">
                        {(() => {
                          const q = memberSearch.trim().toLowerCase();
                          const list = tenantUsers.filter((u) => u.id !== user?.id && (q === "" || u.name.toLowerCase().includes(q) || (u.handle || "").toLowerCase().includes(q)));
                          if (list.length === 0) {
                            return <div className="text-center text-[12px] text-gray-400 py-6">Nenhum usuário encontrado.</div>;
                          }
                          return list.map((u) => {
