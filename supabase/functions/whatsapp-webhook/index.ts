...
async function handleIncomingMessage(
  ...
  try {
    ...
    if (pulse && pulse.pulse_campaigns?.company_id === company_id) {
      ...
      // Honor auto_reply_inbound: still record the inbound but skip AI work
      if (settings && settings.auto_reply_inbound === false) {
        await supabase.from("pulse_outbound_leads").update({
          last_inbound_at: new Date().toISOString(),
          status: "respondeu",
        }).eq("id", pulse.id);
      } else {
        // Flip ai_typing on while we classify
        await supabase.from("pulse_outbound_leads").update({ ai_typing: true }).eq("id", pulse.id);
        ...
        const classifyRes = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/accord-pulse-agent`, {
          ...
          body: JSON.stringify({
            ...
            lead, pulseLead: pulse,
            recentMessages: (recentMsgs || []).reverse(),
          }),
        });
        const cls: any = await classifyRes.json().catch(() => ({}));

        const minD = settings?.min_delay_minutes ?? 3;
        const maxD = settings?.max_delay_minutes ?? 12;
        const delayMin = Math.floor(minD + Math.random() * Math.max(1, maxD - minD));
        const nextAt = (cls.opt_out || cls.needs_human) ? null
        ...
        await supabase.from("pulse_agent_events").insert({
          campaign_id: pulse.campaign_id, pulse_lead_id: pulse.id,
          event_type: "inbound_classified", direction: "inbound",
          message: message || caption || "",
          detected_intent: cls.intent, detected_objection: cls.objection,
          detected_sentiment: cls.sentiment,
          metadata: { needs_human: cls.needs_human, opt_out: cls.opt_out, meeting_requested: cls.meeting_requested },
        });
      }
    }
  } catch (pulseErr) {
