...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { tenant_id, number, numbers } = await req.json();
    ...
    const list: string[] = (
      Array.isArray(numbers) ? numbers : number ? [number] : []
    )
      .map((n: string) => normalizePhone(n))
      .filter(Boolean);

    if (list.length === 0) return json({ error: "no numbers" }, 400);
    ...
    console.error("uazapi-check-number:", e);
