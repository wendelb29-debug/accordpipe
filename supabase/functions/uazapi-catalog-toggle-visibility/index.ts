...
/** Body: { tenant_id, id, hide: boolean } */
Deno.serve(async (req) => {
  ...
  try {
    ...
    const path = hide ? "/business/catalog/hide" : "/business/catalog/show";
    const data = await callUazapi(path, {
      method: "POST",
      token: row.uazapi_token,
      body: { id },
    });
    return json({ ok: true, is_hidden: hide, raw: data });
  } catch (e: any) {
    console.error("uazapi-catalog-toggle-visibility:", e);
