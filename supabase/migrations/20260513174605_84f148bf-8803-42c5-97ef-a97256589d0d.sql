...
-- Revoke broad table grants and re-grant only safe (non-credential) columns.
...
REVOKE SELECT ON public.tenant_whatsapp_integrations FROM authenticated, anon;
GRANT SELECT (
  ...
  provider_metadata
) ON public.tenant_whatsapp_integrations TO authenticated;
